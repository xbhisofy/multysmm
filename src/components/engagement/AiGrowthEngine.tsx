...
export function AiGrowthEngine({ availablePlatforms, selectedPlatform, onPlatformChange, onApply }: Props) {
  ...
  return (
    <div className="relative mb-6">
      {/* Ambient glow */}
      <div className="pointer-events-none absolute -inset-4 -z-10 rounded-3xl bg-gradient-to-r from-blue-200/40 via-fuchsia-200/30 to-fuchsia-200/40 blur-2xl" />
      ...
      <div className="rounded-3xl border border-slate-200/70 bg-white/90 backdrop-blur-xl p-4 sm:p-5 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.15)]">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <div className="flex items-center justify-center w-9 h-9 rounded-full bg-blue-100">
              <Zap className="w-4.5 h-4.5 text-blue-500 fill-blue-500" strokeWidth={2.5} />
            </div>
            <span className="text-[13px] font-bold tracking-[0.14em] text-slate-900">
              AI GROWTH ENGINE
            </span>
          </div>
          <div className="hidden sm:flex items-center gap-1.5 text-[12px] font-semibold text-fuchsia-600">
        ...
        {/* Prompt area */}
        <div className="rounded-2xl border border-slate-200 bg-slate-50/60 focus-within:border-blue-300 focus-within:bg-white focus-within:ring-4 focus-within:ring-blue-100 transition-all">
          <textarea
            value={prompt}
          ...
          <div className="mt-2.5 flex flex-wrap gap-1.5">
            {SAMPLE_PROMPTS.map((s) => (
              <button
                ...
                type="button"
                onClick={() => setPrompt(s)}
                className="text-[11px] px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 hover:bg-blue-50 hover:text-blue-700 transition-colors"
              >
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={cn(
              ...
              "bg-gradient-to-r from-blue-500 via-rose-500 to-fuchsia-500",
